 ...
 * `src/json.ts`) per the same decision the runtime package recorded for
 * blueprint v0.5 §7.6 ("`@deepseek-ai/dsh-session` (peer, only for
 * snapshotJsonValue) → inline"): this plugin's only direct use of that
 * package is detaching JSON at the tool-bridge boundary, so the helper and
 * its validators are vendored and `dashr-tool-presentation` adds no dsh
 * package beyond its declared `@deepseek-ai/dsh-tools` peer. `isJsonValue`
 * (the validation-only sibling export) was not carried over; everything else
 * is verbatim with the runtime package's own copy.
 * @module dashr-tool-presentation/snapshot-json
 */
