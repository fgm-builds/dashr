 ...
 * @module dashr-code-runtime-ipython
 */
...
export class IPythonCodeRuntime extends RLMRuntime {
  ...
  constructor(ctx: Context, config: Config) {
    ...
    for (const key of ['startupTimeoutMs', 'runTimeoutMs', 'interruptGraceMs', 'disposeTimeoutMs', 'snapshotTimeoutMs'] as const) {
      ...
      if (!(Number.isFinite(value) && value > 0)) {
        throw new Error(`dashr-code-runtime-ipython: config.${key} must be a positive number, got ${String(value)}`)
      }
    }
    ...
    if (!(this.config.interruptConfirmMs > 0 && this.config.interruptConfirmMs < this.config.interruptGraceMs)) {
      throw new Error(`dashr-code-runtime-ipython: config.interruptConfirmMs must be positive and below interruptGraceMs (${this.config.interruptGraceMs}), got ${String(this.config.interruptConfirmMs)}`)
    }
    if (!Number.isSafeInteger(this.config.maxOutputBytes) || this.config.maxOutputBytes < MIN_OUTPUT_BYTES) {
      throw new Error(`dashr-code-runtime-ipython: config.maxOutputBytes must be a safe integer of at least ${MIN_OUTPUT_BYTES}, got ${String(this.config.maxOutputBytes)}`)
    }
    if (!Number.isSafeInteger(this.config.snapshotSizeCapBytes) || this.config.snapshotSizeCapBytes < 1) {
      throw new Error(`dashr-code-runtime-ipython: config.snapshotSizeCapBytes must be a positive safe integer, got ${String(this.config.snapshotSizeCapBytes)}`)
    }
    this.logger = ctx.logger('dashr-code-runtime-ipython')
  ...
  async run(request: CodeRunRequest): Promise<CodeRunResult> {
    if (this.disposed) throw new Error('dashr-code-runtime-ipython: run() after disposal')
  ...
  private validateBindings(request: CodeRunRequest): Map<string, CodeBindingNamespace> {
    ...
    for (const namespace of request.bindings) {
      if (!IDENTIFIER.test(namespace.global) || PORTABLE_RESERVED_WORDS.has(namespace.global)) {
        throw new Error(`dashr-code-runtime-ipython: binding global ${JSON.stringify(namespace.global)} is not a usable identifier`)
      }
      if (RESERVED_BINDING_GLOBALS.has(namespace.global) || KERNEL_OWNED_NAME.test(namespace.global)) {
        throw new Error(`dashr-code-runtime-ipython: reserved binding global ${JSON.stringify(namespace.global)}`)
      }
      if (bindings.has(namespace.global)) {
        throw new Error(`dashr-code-runtime-ipython: duplicate binding global ${JSON.stringify(namespace.global)}`)
      }
      ...
      if (namespace.callable === true && Object.keys(namespace.functions).length !== 1) {
        throw new Error(`dashr-code-runtime-ipython: callable binding global ${JSON.stringify(namespace.global)} must declare exactly one function`)
      }
    ...
    for (const namespace of request.bindings) {
      ...
      if (!IDENTIFIER.test(descriptor.name) || PORTABLE_RESERVED_WORDS.has(descriptor.name)) {
        throw new Error(`dashr-code-runtime-ipython: binding error class ${JSON.stringify(descriptor.name)} is not a usable identifier`)
      }
      if (RESERVED_BINDING_GLOBALS.has(descriptor.name) || KERNEL_OWNED_NAME.test(descriptor.name)) {
        throw new Error(`dashr-code-runtime-ipython: reserved binding global ${JSON.stringify(descriptor.name)}`)
      }
      if (bindings.has(descriptor.name) || errorClassNames.has(descriptor.name)) {
        throw new Error(`dashr-code-runtime-ipython: duplicate injected global ${JSON.stringify(descriptor.name)}`)
      }
      const member = descriptor.memberNameProperty
      if (member.length === 0 || RESERVED_ERROR_MEMBERS.has(member) || DUNDER_MEMBER.test(member)) {
        throw new Error(`dashr-code-runtime-ipython: binding error member property ${JSON.stringify(descriptor.memberNameProperty)} is not usable`)
      }
